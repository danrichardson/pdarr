# Configuration Manifest — PDARR Transcoder
**Generated:** 2026-04-02
**Source transcript:** c:\src\_project-pdarr\docs\downloaded-prompts\dan-transcript.md
...
4. **Settings UI vs. config file boundary** — Founder said "configure directories on my location... when it meets the criteria that I set." It was never resolved which settings live in the web UI vs. a config file (e.g., pdarr.toml). Specifically: concurrency, disk threshold, scan interval, Plex URL/token.
...
6. **Name finalization** — Founder confirmed "PDARR" (p-d-a-r-r) but tagline was not resolved. Founder suggested "save space R Us" and "transcode your shit and it works" as jokes; no final tagline locked.
