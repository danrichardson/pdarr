# WORKFLOW.md
# PDARR — Media Transcoder

---

## Vision

PDARR is a self-hosted media transcoding service that automatically shrinks bloated video files using hardware GPU encoding. It replaces Tdarr — a tool that was overcomplicated, brittle, and failed in production. PDARR is everything Tdarr should have been: focused, reliable, polished, and open source from day one. A single Go binary, a clean React admin panel, no Docker, no subscriptions, no bullshit.
...
PDARR does not use environment variables for runtime config — it uses a TOML config file. The `.env`-style values below map to config file sections. Copy to `pdarr.toml` and fill in values.
...
port = 8080
data_dir = "/var/lib/pdarr"   # where pdarr.db lives
...
quarantine_dir = ""   # empty = .pdarr-quarantine/ adjacent to source
...
# To set: run `pdarr hash-password` and paste the output here
password_hash = ""
...
- [ ] Create GitHub repo `danrichardson/pdarr` with `main` branch protection, PR template, issue labels (bug, enhancement, wontfix)
- [ ] Scaffold `.github/workflows/build.yml` — lint + test on every push to main; confirm it passes on empty project
- [ ] Decide on quarantine directory naming: `.pdarr-quarantine` adjacent to source vs. single configurable global path. **Assumption**: global configurable path, default: `{data_dir}/quarantine/`. Flag for owner review.
...
1. Go module init: `github.com/danrichardson/pdarr`. Directory structure:
   ```
   cmd/pdarr/main.go
   ...
   ```
2. Config loader: parse `pdarr.toml`, validate required fields, fail fast with clear error if invalid
...
10. CLI: `pdarr serve`, `pdarr scan-once`, `pdarr hash-password`
...
- `pdarr serve` starts, scans configured directory, transcodes one qualifying file, replaces it
- `pdarr.db` contains a completed job with `status = 'done'` and non-null `bytes_saved`
...
4. Restore: `pdarr restore {job_id}` — moves original back from quarantine, deletes transcoded file, marks job as cancelled
5. Add `--dry-run` flag to `pdarr scan-once` for safe testing
...
3. launchd plist: `scripts/com.pdarr.agent.plist`
4. macOS install script: `scripts/install-macos.sh` — copies binary to `/usr/local/bin`, config to `~/.config/pdarr/`, installs plist
...
```
You are continuing work on PDARR, a self-hosted media transcoding service.

Project: github.com/danrichardson/pdarr
...
| 2 | modernc.org/sqlite (pure Go) over mattn/go-sqlite3 (CGo) | Simpler cross-compilation to darwin/arm64; no CGo toolchain required | Flag: verify performance is acceptable for the query patterns PDARR uses |
| 3 | SQLite over PostgreSQL/MySQL | Single-instance app, no network DB, embedded is the right call | No |
| 4 | Quarantine dir defaults to global `{data_dir}/quarantine/` not `.pdarr-quarantine` adjacent to source | Cleaner, avoids polluting media directories, easier to configure a separate fast disk | Flag: owner may prefer adjacent-to-source for clarity |
...
| 18 | Codec-based skip removed; bitrate is the only gate | Owner's actual requirement: "don't store more than ~1 GB/hr for a TV show." A 5 GB HEVC file is still oversized and must be transcoded. The original HEVC skip was the interviewer's assumption, not the owner's requirement. | No |
| 19 | Default `max_bitrate` changed from 4,000,000 to 2,222,000 bps | 2,222,000 bps ≈ 1 GB/hour, matching the owner's stated threshold. 4 Mbps was ~1.8 GB/hr — too permissive. | No |
| 20 | `disk_free_pause_gb` feature removed entirely | Owner never requested a pause threshold. The whole point is to compress bloated files to reclaim space — pausing when full is counterproductive. Replaced with a per-job temp space pre-check: if there isn't 1.2× the source file size free, skip that job and continue with the next. | No |
| 21 | "Quarantine" renamed to "Processed" in UI | "Quarantine" implies anti-virus / threat containment. The feature holds originals temporarily while the transcoded version is validated — "Processed" is accurate and non-alarming. Backend API routes retain `/quarantine` for stability. | No |
| 22 | Fix ZFS write permission via targeted `chown` on PVE host, not by making container privileged | Discovered in production: even root inside an unprivileged LXC cannot write to ZFS-backed bindmounts. UID 0 in the container maps to UID 100000 on the host; the ZFS dataset was owned by nobody/nogroup. Fix: `chown -R 100000:100000 /storePool/subvol-250-disk-0/srv/storage/Videos` on PVE2 — scoped to Videos only, not the whole pool. Container stays unprivileged. | No |
| 23 | pdarr service runs as root (`User=root`) | Required by decision 22. When the container is privileged and the ZFS mount is writable by root, this is the simplest path. A dedicated user could work if the media directories are chowned to that UID on the host, but adds operational complexity with no security gain in a single-user home server context. | No |
| 24 | SPA routing: serve index.html for all unmatched paths | Initial spaHandler just forwarded all requests to the file server, returning 404 for any non-root path on hard refresh. Fixed by checking if the requested path exists as a real static file; if not, serve index.html and let the React router handle it. | No |
| 25 | ffmpeg stderr fully captured and stored in `error_message` | Initial implementation only stored "ffmpeg: exit status 243" — useless for diagnosis. Now the full stderr is collected (with progress-only lines stripped), and stored in the DB. The History UI renders it as a scrollable preformatted block. First failure diagnosed this way revealed the permission issue (decision 22). | No |
| 26 | Disk space widget measures first configured media directory, not data_dir | `/var/lib/pdarr` (the data dir) is on the system disk; the number that matters is free space on the media drive. The widget now stats the first enabled directory and shows the path it's measuring. | No |
| 27 | File browser in Queue page shows video files, not just directories | The manual "Queue a file" form needed a way to browse to a specific file. The existing `/api/v1/fs` endpoint was extended with `?files=1` to also return video files (mkv, mp4, avi, etc.) alongside directories. | No |
| 28 | Build-on-server workflow (no Go on Windows build machine) | Go was not installed on the Windows dev machine. Source is scp'd to `/opt/pdarr-src` on the server and built there with `go mod tidy && go build`. The `go.sum` is generated server-side and not committed. This is the current deploy workflow until a proper CI release pipeline is set up. | No |
| 29 | `file_size` column added to `quarantine` table post-initial-deploy | The schema migration was missed during initial deploy. Existing installations must run: `ALTER TABLE quarantine ADD COLUMN file_size INTEGER NOT NULL DEFAULT 0;` before starting a binary that includes this column. Documented in README migration table. | No |
