...
| 26 | Disk space widget measures first configured media directory, not data_dir | `/var/lib/sqzarr` (the data dir) is on the system disk; the number that matters is free space on the media drive. The widget now stats the first enabled directory and shows the path it's measuring. | No |
...
| 28 | Build-on-server workflow (no Go on Windows build machine) | Go was not installed on the Windows dev machine. Source is scp'd to `/opt/sqzarr-src` on the server and built there with `go mod tidy && go build`. The `go.sum` is generated server-side and not committed. This is the current deploy workflow until a proper CI release pipeline is set up. | No |
