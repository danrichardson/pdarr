You are an expert full-stack developer. You produce complete, ready-to-use starter code.
...
Based on the interview transcript and architecture decisions, produce a complete code starter packet. For each file, use this format:
...
- wrangler.jsonc or equivalent deployment config
- Main entry point / worker
- Database schema SQL
- .env.example with all required keys (no values)
- GitHub Actions workflow files (staging + prod deploy)
- Smoke test script
- README.md with setup instructions

Every file should be complete and correct — no TODOs, no placeholders. A developer should be able to copy these files, run npm install, fill in secrets, and deploy.