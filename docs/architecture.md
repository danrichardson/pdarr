# PDARR — Technical Architecture Document

## Executive Summary

PDARR is a self-hosted media transcoding daemon with a React admin panel. It solves one problem: bloated video files on a NAS or home server that should be re-encoded to HEVC using available GPU hardware, safely and automatically, without any manual babysitting.
...
graph TB
    ...
    subgraph "PDARR Daemon (Go binary)"
        SC[Scanner]
        RE[Rules Engine]
        JQ[(Job Queue\nSQLite)]
        TW[Transcode Worker]
...
**Tradeoffs**: Not suitable for multi-instance deployment — acceptable because PDARR is explicitly single-instance.  
...
- Config file permissions: `chmod 600 pdarr.toml`
- No secrets logged
...
└── LXC Container (Debian 12, unprivileged)
    ├── /usr/local/bin/pdarr          (the binary)
    ├── /etc/pdarr/pdarr.toml         (config)
    ├── /var/lib/pdarr/pdarr.db       (SQLite)
    ├── /var/log/pdarr/               (log files)
    └── systemd: pdarr.service

    Hardware passthrough:
    └── /dev/dri/renderD128           (Intel GPU via LXC device cgroup)
```

**systemd unit** (`/etc/systemd/system/pdarr.service`):
```ini
[Unit]
...
macOS (Apple Silicon)
├── /usr/local/bin/pdarr
├── ~/.config/pdarr/pdarr.toml
├── ~/Library/Application Support/pdarr/pdarr.db
└── launchd: ~/Library/LaunchAgents/com.pdarr.agent.plist
```
...
PDARR is explicitly a single-instance, single-user application. It does not need to scale horizontally.
...
**Future multi-machine**: not in scope. If needed, the right answer is multiple independent PDARR instances on separate machines, not distributed coordination.
...
5. **ffmpeg subprocess**: Runs as the `pdarr` service user, which should only have read access to media directories and write access to the quarantine directory. systemd `ProtectSystem=strict`, `ReadWritePaths=` restrict file system access.

6. **SQLite**: DB file should be owned by `pdarr:pdarr` with permissions `640`.
...
- Go project structure: `cmd/pdarr`, `internal/scanner`, `internal/transcoder`, `internal/db`, `internal/config`
