#!/usr/bin/env bash
# PDARR smoke test — starts daemon, adds directory, triggers scan,
# waits for one completed job, confirms DB state. Exits 0 on success.
set -euo pipefail

BINARY="${PDARR_BINARY:-./pdarr}"
...
WORKDIR=$(mktemp -d)
trap "rm -rf $WORKDIR; kill $PDARR_PID 2>/dev/null || true" EXIT
...
echo "Starting pdarr..."
LIBVA_DRIVER_NAME=iHD "$BINARY" serve -config "$CONFIG" > "$WORKDIR/pdarr.log" 2>&1 &
PDARR_PID=$!
sleep 2
