Set-Location 'c:\src\_project-pdarr'
...
$proc = Start-Process -FilePath $goExe `
    -ArgumentList 'build', '-trimpath', '-ldflags=-s -w', '-o', 'dist/pdarr-linux-amd64', './cmd/pdarr/' `
    -Wait -PassThru -NoNewWindow
Write-Host "Exit: $($proc.ExitCode)"
if ($proc.ExitCode -eq 0) {
    $size = (Get-Item 'dist\pdarr-linux-amd64').Length
    Write-Host "Built: dist/pdarr-linux-amd64 ($([math]::Round($size/1MB, 1)) MB)"
} else {
    exit 1
}
