...
func (db *DB) ListDirectories() ([]*Directory, error) {
	rows, err := db.conn.Query(`
		SELECT id, path, enabled, min_age_days, max_bitrate, min_size_mb, created_at, updated_at
	...
	for rows.Next() {
		...
		if err := rows.Scan(&d.ID, &d.Path, &d.Enabled, &d.MinAgeDays,
			&d.MaxBitrate, &d.MinSizeMB, &d.CreatedAt, &d.UpdatedAt); err != nil {
...
func (db *DB) GetDirectory(id int64) (*Directory, error) {
	...
	err := db.conn.QueryRow(`
		...
		Scan(&d.ID, &d.Path, &d.Enabled, &d.MinAgeDays,
			&d.MaxBitrate, &d.MinSizeMB, &d.CreatedAt, &d.UpdatedAt)
...
func (db *DB) InsertDirectory(d *Directory) (int64, error) {
	res, err := db.conn.Exec(`
		INSERT INTO directories (path, enabled, min_age_days, max_bitrate, min_size_mb)
		VALUES (?, ?, ?, ?, ?)`,
		d.Path, d.Enabled, d.MinAgeDays, d.MaxBitrate, d.MinSizeMB)
	if err != nil {
...
func (db *DB) UpdateDirectory(d *Directory) error {
	_, err := db.conn.Exec(`
		UPDATE directories
		SET path=?, enabled=?, min_age_days=?, max_bitrate=?, min_size_mb=?, updated_at=CURRENT_TIMESTAMP
		WHERE id=?`,
		d.Path, d.Enabled, d.MinAgeDays, d.MaxBitrate, d.MinSizeMB, d.ID)
	if err != nil {
