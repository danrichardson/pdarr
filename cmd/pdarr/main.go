...
const usage = `pdarr — self-hosted GPU media transcoder

Usage:
  pdarr serve              Start the HTTP server and worker daemon
  pdarr scan-once          Run a single scan pass and exit
  pdarr restore <job-id>   Restore original file from quarantine
  pdarr hash-password      Hash a password for use in pdarr.toml

Flags:
  -config string   Path to pdarr.toml (default: /etc/pdarr/pdarr.toml)
`
...
func main() {
	...
	cfgPath := flag.String("config", "/etc/pdarr/pdarr.toml", "path to pdarr.toml")
	...
	case "restore":
		...
		args := flag.Args()
		if len(args) < 1 {
			fmt.Fprintln(os.Stderr, "usage: pdarr restore <job-id>")
			os.Exit(1)
		}
